...
def main():
    ...
    examples_file_path = os.path.join('./Dataset', '50 Examples.xlsx')
...
Please now rate the following reviews:\n"""
    ...
    directory_path = os.path.join('./Dataset', product_id, model, 'finetune')
    ...
    combined_df.to_csv(
        './Dataset' + "/{}".format(product_id) + "/{}".format(model) + "/finetune" + "/{}.csv".format(datetime.now()),
        index=False)
